---
id: table-of-contents
sidebar_position: 1
---

# Table of Contents

The [TypeDoc documentation for SciChart.js can be found here:blue_book:](https://www.scichart.com/documentation/js/v5/typedoc/index.html).

<!-- generate-docs-toc.js start -->
- **User Manual**
  - [Table of Contents](/user-manual/toc)
  - [Features, Chart Types and more supported by SciChart.js](/user-manual/what-chart-types)
  - [Minimum Browser Requirements](/user-manual/minimum-browser-requirements)
  - [Online Resources and Help](/user-manual/online-resources-and-help)
  - **Licensing SciChart.js**
    - [Get Started here](/user-manual/licensing-scichart-js/getting-started-for-free)
    - [Location & Management of Licenses after Purchase](/user-manual/licensing-scichart-js/where-are-my-license-keys)
    - [Activating a Purchased Developer License](/user-manual/licensing-scichart-js/activating-paid-licenses)
    - [Deploying SciChart.js to Domains](/user-manual/licensing-scichart-js/deploying-to-live-sites)
    - [Deploying SciChart.js with Advanced Licensing (OEM)](/user-manual/licensing-scichart-js/deploying-with-advanced-licensing)
    - [Github Source Code Access](/user-manual/licensing-scichart-js/github-source-code-access)
    - [SciChart.js Licensing Troubleshooting](/user-manual/licensing-scichart-js/licensing-troubleshooting)
    - [Misc licensing faqs](/user-manual/licensing-scichart-js/misc-licensing-faq)
  - **Language Support**
    - [SciChart 是否支持中文](/user-manual/language-support/chinese)
    - [SciChartは日本語をサポートしていますか？](/user-manual/language-support/japanese)
- **Get Started: Tutorials, Examples**
  - **Tutorials (JavaScript APIs / npm / webpack)**
    - [Tutorial 01 - Setting up a npm Project with SciChart.js](/get-started/tutorials-js-npm-webpack/tutorial-01-setting-up-npm-project-with-scichart-js)
    - [Tutorial 02 - Adding Series and Data](/get-started/tutorials-js-npm-webpack/tutorial-02-adding-series-and-data)
    - [Tutorial 03 - Adding Zooming, Panning Behavior](/get-started/tutorials-js-npm-webpack/tutorial-03-adding-zooming-panning-behavior)
    - [Tutorial 04 - Adding Realtime Updates](/get-started/tutorials-js-npm-webpack/tutorial-04-adding-realtime-updates)
    - [Tutorial 05 - Zoom and Pan with Realtime Updates](/get-started/tutorials-js-npm-webpack/tutorial-05-zoom-and-pan-with-realtime-updates)
    - [Tutorial 06 - Adding Annotations](/get-started/tutorials-js-npm-webpack/tutorial-06-adding-annotations)
    - [Tutorial 07 - Adding Tooltips and Legends](/get-started/tutorials-js-npm-webpack/tutorial-07-adding-tooltips-and-legends)
    - [Tutorial 08 - Adding Multiple Axis](/get-started/tutorials-js-npm-webpack/tutorial-08-adding-multiple-axis)
    - [Tutorial 09 - Linking Multiple Charts](/get-started/tutorials-js-npm-webpack/tutorial-09-linking-multiple-charts)
    - [Tutorial 10 - Vertical Charts](/get-started/tutorials-js-npm-webpack/tutorial-10-vertical-charts)
  - **Tutorials (index.min.js from CDN)**
    - [Tutorial 01 - Including SciChart.js in an HTML Page using CDN](/get-started/tutorials-cdn/tutorial-01-using-cdn)
    - [Tutorial 02 - Including index.min.js and WebAssembly Files offline](/get-started/tutorials-cdn/tutorial-02-offline)
  - **Tutorials (scichart-react)**
    - [Tutorial 01 - Setting up a project with scichart-react and config object](/get-started/tutorials-react/tutorial-01-setting-up-project-with-scichart-react)
    - [Tutorial 02 - Creating a Chart with scichart-react](/get-started/tutorials-react/tutorial-02-creating-a-chart-with-scichart-react)
    - [Tutorial 03 - Modifying Chart Data and Behavior in React](/get-started/tutorials-react/tutorial-03-modifying-chart-data-and-behavior-in-react)
    - [Tutorial 04 - Adding & Removing Charts To a Group in React](/get-started/tutorials-react/tutorial-04-adding-and-removing-charts-to-a-group-in-react)
    - [Tutorial 05 - Synchronizing React Charts with Data in a Group](/get-started/tutorials-react/tutorial-05-synchronizing-react-charts-with-data-in-a-group)
  - [The SciChart.js Examples Suite](/get-started/scichart-js-examples-suite)
  - **FAQs**
    - [Virtualized data example JS RxJS from CDN without NPM](/get-started/faqs/faq-20250722)
    - [Custom Rollover SVG Tooltips](/get-started/faqs/faq-customTooltips)
    - [Custom Side-By-Side Series](/get-started/faqs/faq-sideBySideSeries)
    - [How to Detect If a User Is Zooming or Panning](/get-started/faqs/faq-zoomState)
    - [Hover and Click Interactions](/get-started/faqs/faq_hover_interactions)
    - [FAQ: Generating a Standalone SciChart HTML File](/get-started/faqs/faq_standaloneHTMLfile)
- **What's New?**
  - [What's New in SciChart.js SDK v5.0](/whats-new/sdk-5.0)
  - [What's New in SciChart.js SDK v4.0](/whats-new/sdk-4.0)
  - [Breaking Changes in SciChart.js v5.x from v4.x](/whats-new/breaking-changes-v4-v5)
  - [Breaking Changes in SciChart.js v4.x from v3.x](/whats-new/breaking-changes-v3-v4)
- **2D Charts API**
  - **The SciChartSurface**
    - [The SciChartSurface Type](/2d-charts/surface/scichart-surface-type-overview)
    - [The SciChartPolarSurface Type](/2d-charts/surface/scichart-polar-surface-type)
    - [Creating a new SciChartSurface and loading Wasm](/2d-charts/surface/new-scichart-surface)
    - [Setting a Runtime License on a SciChartSurface](/2d-charts/surface/runtime-license)
    - [Deploying Wasm (WebAssembly) with your app](/2d-charts/surface/deploying-wasm)
  - **2D Chart Types**
    - [RenderableSeries Overview](/2d-charts/chart-types/renderable-series-api-overview)
    - **Common Series APIs**
      - [Series isVisible and isVisibleChanged API](/2d-charts/chart-types/common-series-apis/is-visible)
      - [Drawing Gaps in Series (null data)](/2d-charts/chart-types/common-series-apis/drawing-gaps)
      - [Drawing PointMarkers on Series (Scatter markers)](/2d-charts/chart-types/common-series-apis/drawing-point-markers)
    - **DataLabels API**
      - [DataLabels API Overview](/2d-charts/chart-types/data-point-labels/data-labels-api-overview)
      - [Data Label Positioning](/2d-charts/chart-types/data-point-labels/data-label-positioning)
      - [Data Label Coloring](/2d-charts/chart-types/data-point-labels/data-label-coloring)
      - [Custom DataLabel Formatting with getText()](/2d-charts/chart-types/data-point-labels/custom-data-label-formatting)
      - [Getting Labels from Metadata](/2d-charts/chart-types/data-point-labels/getting-labels-from-metadata)
      - [DataLabel SkipModes and Culling](/2d-charts/chart-types/data-point-labels/data-label-skip-modes-and-culling)
      - [Polar Data Point Labels](/2d-charts/chart-types/data-point-labels/polar-data-point-labels)
    - **PaletteProvider API**
      - [PaletteProvider API Overview](/2d-charts/chart-types/palette-provider-api/palette-provider-api-overview)
      - [Per-point Colouring for Line Series](/2d-charts/chart-types/palette-provider-api/fast-line-renderable-series)
      - [Per-point Colouring of Mountain Segments](/2d-charts/chart-types/palette-provider-api/fast-mountain-renderable-series)
      - [Per-Point Colouring of Band Segments](/2d-charts/chart-types/palette-provider-api/fast-band-renderable-series)
      - [Per-Point Colouring of Bubble Charts](/2d-charts/chart-types/palette-provider-api/fast-bubble-renderable-series)
      - [Per-Point Colouring of Candlestick / OHLC Charts](/2d-charts/chart-types/palette-provider-api/fast-candlestick-ohlc-renderable-series)
      - [Per-Point Colouring of Column Charts](/2d-charts/chart-types/palette-provider-api/fast-column-renderable-series)
      - [Per-Point Colouring of Scatter Charts (or PointMarkers)](/2d-charts/chart-types/palette-provider-api/xy-scatter-renderable-series)
      - [Per-Point Colouring of Impulse Charts](/2d-charts/chart-types/palette-provider-api/fast-impulse-renderable-series)
      - [Per-Point Coloring for Rectangle Series](/2d-charts/chart-types/palette-provider-api/fast-rectangle-renderable-series)
      - [Per-Point Coloring for Line Segment Series](/2d-charts/chart-types/palette-provider-api/fast-line-segment-renderable-series)
      - [Per-Point Coloring for Triangle Series](/2d-charts/chart-types/palette-provider-api/fast-triangle-renderable-series)
      - [Per-Point Coloring for Polar Band Series](/2d-charts/chart-types/palette-provider-api/polar-band-renderable-series)
      - [Per-Point Coloring for Polar Column Series](/2d-charts/chart-types/palette-provider-api/polar-column-renderable-series)
      - [Per-Point Coloring for Polar Line Series](/2d-charts/chart-types/palette-provider-api/polar-line-renderable-series)
      - [Per-Point Coloring for Polar Scatter Series](/2d-charts/chart-types/palette-provider-api/polar-xy-scatter-renderable-series)
      - [PaletteProvider for Stacked Columns](/2d-charts/chart-types/palette-provider-api/palette-provider-stacked-columns)
      - [The PaletteFactory Helper Class](/2d-charts/chart-types/palette-provider-api/palette-factory)
    - **Hit-Test API**
      - [Hit-Test API overview](/2d-charts/chart-types/hit-test-api/hit-test-api-overview)
      - [Hit-Test API for Band Series](/2d-charts/chart-types/hit-test-api/fast-band-renderable-series)
      - [Hit-Test API for Line Series](/2d-charts/chart-types/hit-test-api/fast-line-renderable-series)
      - [Hit-Test API for Bubble Series](/2d-charts/chart-types/hit-test-api/fast-bubble-renderable-series)
      - [Hit-Test API for Column Series](/2d-charts/chart-types/hit-test-api/fast-column-renderable-series)
      - [Hit-Test API for Heatmap Series](/2d-charts/chart-types/hit-test-api/uniform-heatmap-renderable-series)
      - [Hit-Test API for Mountain Series](/2d-charts/chart-types/hit-test-api/fast-mountain-renderable-series)
      - [Hit-Test API for Candlestick and OHLC Series](/2d-charts/chart-types/hit-test-api/fast-candlestick-ohlc-renderable-series)
      - [Hit-Test API for Scatter Series](/2d-charts/chart-types/hit-test-api/xy-scatter-renderable-series)
      - [Hit-Test API for Stacked Column Series](/2d-charts/chart-types/hit-test-api/stacked-column-renderable-series)
      - [Hit-Test API for Stacked Mountain Series](/2d-charts/chart-types/hit-test-api/stacked-mountain-renderable-series)
      - [Hit-Test API for Rectangle Series](/2d-charts/chart-types/hit-test-api/fast-rectangle-renderable-series)
      - [Hit-Test API for Line Segment Series](/2d-charts/chart-types/hit-test-api/fast-line-segment-renderable-series)
      - [Hit-Test API for Triangle Series](/2d-charts/chart-types/hit-test-api/fast-triangle-renderable-series)
      - [Hit-Test API for Polar Band Series](/2d-charts/chart-types/hit-test-api/polar-band-renderable-series)
      - [Hit-Test API for Polar Column Series](/2d-charts/chart-types/hit-test-api/polar-column-renderable-series)
      - [Hit-Test API for Polar Line Series](/2d-charts/chart-types/hit-test-api/polar-line-renderable-series)
      - [Hit-Test API for Polar Mountain Series](/2d-charts/chart-types/hit-test-api/polar-mountain-renderable-series)
      - [Hit-Test API for Polar Stacked Column Series](/2d-charts/chart-types/hit-test-api/polar-stacked-column-renderable-series)
      - [Hit-Test API for Polar Stacked Mountain Series](/2d-charts/chart-types/hit-test-api/polar-stacked-mountain-renderable-series)
      - [Hit-Test API for Polar Uniform Heatmap Renderable Series](/2d-charts/chart-types/hit-test-api/polar-uniform-heatmap-renderable-series)
      - [Hit-Test API for Polar XY Scatter Series](/2d-charts/chart-types/hit-test-api/polar-xy-scatter-renderable-series)
      - [Hit-Test API and Metadata](/2d-charts/chart-types/hit-test-api/metadata)
    - **DataSeries (Data Updates) API**
      - [DataSeries API Overview](/2d-charts/chart-types/data-series-api/data-series-api-overview)
      - [Append, Insert, Update, Remove](/2d-charts/chart-types/data-series-api/append-insert-update-remove)
      - [Accessing DataSeries xValues, yValues and count](/2d-charts/chart-types/data-series-api/get-set-value-at-index)
      - [DataSeries Realtime Updates](/2d-charts/chart-types/data-series-api/realtime-updates)
      - [DataSeries Resampling](/2d-charts/chart-types/data-series-api/resampling)
      - [Deleting DataSeries Memory](/2d-charts/chart-types/data-series-api/deleting-memory)
    - **PointMetadata API**
      - [PointMetadata API](/2d-charts/chart-types/point-metadata-api/point-metadata-api-overview)
      - [Metadata and PaletteProviders](/2d-charts/chart-types/point-metadata-api/palette-providers)
      - [Metadata and Tooltips](/2d-charts/chart-types/point-metadata-api/tooltips)
      - [Metadata and HitTest](/2d-charts/chart-types/point-metadata-api/hit-test)
    - **Data Filters (Transforms) API**
      - [Data Filters API](/2d-charts/chart-types/data-filters-api/data-filters-api-overview)
      - [Scale Offset Filters](/2d-charts/chart-types/data-filters-api/scale-offset-filters)
      - [Linear Trendline Filter](/2d-charts/chart-types/data-filters-api/linear-trendline-filter)
      - [Moving Average Filter](/2d-charts/chart-types/data-filters-api/moving-average-filter)
      - [Ratio Filter](/2d-charts/chart-types/data-filters-api/ratio-filter)
      - [Creating a Custom Filter](/2d-charts/chart-types/data-filters-api/custom-filter)
    - [RenderDataTransforms API](/2d-charts/chart-types/render-data-transforms-api)
    - [The Line Series Type](/2d-charts/chart-types/fast-line-renderable-series)
    - [The Digital (Step) Line Series](/2d-charts/chart-types/fast-line-digital-renderable-series)
    - [The Scatter Series Type](/2d-charts/chart-types/xy-scatter-renderable-series)
    - [The Mountain (Area) Series Type](/2d-charts/chart-types/fast-mountain-area-renderable-series)
    - [The Digital (Step) Mountain Series Type](/2d-charts/chart-types/fast-mountain-area-digital-renderable-series)
    - **Column Series**
      - [The Column Series Type](/2d-charts/chart-types/fast-column-renderable-series/column-series-type)
      - [Column Series Data Point Width Mode](/2d-charts/chart-types/fast-column-renderable-series/data-point-width-mode)
    - **Uniform Heatmap Series**
      - [The Uniform Heatmap Chart Type](/2d-charts/chart-types/uniform-heatmap-renderable-series/uniform-heatmap-chart-type)
      - [Updating (Realtime) Heatmaps](/2d-charts/chart-types/uniform-heatmap-renderable-series/updating-realtime)
      - [Heatmap ColorMaps and Legends](/2d-charts/chart-types/uniform-heatmap-renderable-series/color-maps-and-legends)
    - [The Non-Uniform Heatmap Chart Type](/2d-charts/chart-types/non-uniform-heatmap-renderable-series)
    - [The Contours Series Type](/2d-charts/chart-types/uniform-contours-renderable-series)
    - [The Candlestick Series type](/2d-charts/chart-types/fast-candlestick-renderable-series)
    - [The OHLC Series Type](/2d-charts/chart-types/fast-ohlc-renderable-series)
    - [The Lollipop (Impulse or Stem) Chart Type](/2d-charts/chart-types/fast-impulse-renderable-series)
    - [The Error Bars Chart Type](/2d-charts/chart-types/fast-error-bars-renderable-series)
    - [The Band Series type](/2d-charts/chart-types/fast-band-renderable-series)
    - [The Digital (Step) Band Series Type](/2d-charts/chart-types/fast-band-digital-renderable-series)
    - [The Bubble Series Type](/2d-charts/chart-types/fast-bubble-renderable-series)
    - [The Fan Charts Type](/2d-charts/chart-types/fan-charts-type)
    - [The Rectangle Series Type](/2d-charts/chart-types/fast-rectangle-renderable-series)
    - [The Triangle Series Type](/2d-charts/chart-types/fast-triangle-renderable-series)
    - [The Line Segment Series Type](/2d-charts/chart-types/fast-line-segment-renderable-series)
    - [The Treemap Chart Type](/2d-charts/chart-types/tree-map-type)
    - [The Stacked Column Series Type](/2d-charts/chart-types/stacked-column-renderable-series)
    - [The Stacked Mountain Series Type](/2d-charts/chart-types/stacked-mountain-renderable-series)
    - [The Pie Chart Type](/2d-charts/chart-types/pie-chart-type)
    - [The Donut Chart Type](/2d-charts/chart-types/donut-chart-type)
    - [The Spline (Smoothed) Line Series Type](/2d-charts/chart-types/spline-line-renderable-series)
    - [The Spline (Smoothed) Mountain Series Type](/2d-charts/chart-types/spline-mountain-renderable-series)
    - [The Spline (Smoothed) Band Series Type](/2d-charts/chart-types/spline-band-renderable-series)
    - [The Bezier (Smoothed) Stacked Mountain Series Type](/2d-charts/chart-types/smooth-stacked-mountain-renderable-series)
    - [The Text Series Type](/2d-charts/chart-types/fast-text-renderable-series)
    - [The Box Plot Series Type](/2d-charts/chart-types/fast-box-plot-renderable-series)
    - [The Partial Polar Chart Type](/2d-charts/chart-types/polar-partial-chart-type)
    - [The Polar Gauge Chart Type](/2d-charts/chart-types/polar-gauge-chart)
    - [The Polar Band Series Type](/2d-charts/chart-types/polar-band-renderable-series)
    - [The Polar Column Chart Type](/2d-charts/chart-types/polar-column-renderable-series)
    - [The Polar Line Chart Type](/2d-charts/chart-types/polar-line-renderable-series)
    - [The Polar Mountain Chart Type](/2d-charts/chart-types/polar-mountain-renderable-series)
    - [The Polar Stacked Column Chart Type](/2d-charts/chart-types/polar-stacked-column-renderable-series)
    - [The Polar Stacked Mountain Chart Type](/2d-charts/chart-types/polar-stacked-mountain-renderable-series)
    - [The Polar Uniform Heatmap Chart Type](/2d-charts/chart-types/polar-uniform-heatmap-renderable-series)
    - [The Polar Scatter Chart Type](/2d-charts/chart-types/polar-xy-scatter-renderable-series)
    - [The Pie Chart (using Polar Columns)](/2d-charts/chart-types/polar-pie-chart)
    - [The Polar Radar Chart Type](/2d-charts/chart-types/polar-radar-chart)
    - [The Polar Text Series Type](/2d-charts/chart-types/polar-text-renderable-series)
    - [The Polar Triangle Series Type](/2d-charts/chart-types/polar-triangle-renderable-series)
    - [The Polar Sunburst Chart Type](/2d-charts/chart-types/polar-sunburst-chart)
  - **Builder (JSON Chart Definition) API**
    - [Intro to the Builder API](/2d-charts/builder-api/builder-api-overview)
    - [Creating a Simple Chart](/2d-charts/builder-api/simple-chart)
    - [Creating a 3D Chart](/2d-charts/builder-api/default-3d-chart)
    - [Creating a Pie Chart](/2d-charts/builder-api/pie-chart)
    - [Creating a Polar Chart](/2d-charts/builder-api/polar-chart)
    - [Working with Data](/2d-charts/builder-api/working-with-data)
    - [Complex Options](/2d-charts/builder-api/complex-options)
    - [Custom Subtypes](/2d-charts/builder-api/custom-subtypes)
    - [Serialization and Deserialization of Charts](/2d-charts/builder-api/charts-serialization-deserialization)
  - **Axis APIs**
    - [Axis API Overview](/2d-charts/axis-api/axis-api-overview)
    - **Axis Types**
      - [Common Axis Base Type and Options](/2d-charts/axis-api/axis-types/common-axis-base-type)
      - [The Numeric Axis](/2d-charts/axis-api/axis-types/numeric-axis)
      - [The DateTimeNumericAxis](/2d-charts/axis-api/axis-types/date-time-numeric-axis)
      - [The Logarithmic Axis](/2d-charts/axis-api/axis-types/logarithmic-axis)
      - [The Category Axis](/2d-charts/axis-api/axis-types/category-axis)
      - [Text / String Axis](/2d-charts/axis-api/axis-types/text-string-axis)
      - [The Polar Numeric Axis](/2d-charts/axis-api/axis-types/polar-numeric-axis)
      - [The Polar Category Axis](/2d-charts/axis-api/axis-types/polar-category-axis)
      - [Base Value Axis](/2d-charts/axis-api/axis-types/base-value-axis)
      - [Discontinuous Date Axis](/2d-charts/axis-api/axis-types/discontinuous-date-axis)
    - **Axis Ranging, Scaling**
      - [Axis Ranging - AutoRange](/2d-charts/axis-api/ranging-scaling/auto-range)
      - [Axis Ranging - Set Range and Zoom to Fit](/2d-charts/axis-api/ranging-scaling/set-range-zoom-to-fit)
      - [Axis Ranging - How to Listen to VisibleRange Changes](/2d-charts/axis-api/ranging-scaling/listen-to-visible-range-changes)
    - **Axis Labels**
      - [Axis LabelProvider API Overview](/2d-charts/axis-api/axis-labels/label-provider-api-overview)
      - [Axis Labels - The ENumericFormat Enum](/2d-charts/axis-api/axis-labels/numeric-formats)
      - [Custom LabelProviders: Dynamic Dates on Zoom](/2d-charts/axis-api/axis-labels/custom-label-providers-dynamic-dates-on-zoom)
      - [Custom LabelProviders: Readable Numbers](/2d-charts/axis-api/axis-labels/custom-label-providers-readable-numbers)
      - [Rotating Axis Labels](/2d-charts/axis-api/axis-labels/rotating-axis-labels)
      - [Text and Multi-Line Labels](/2d-charts/axis-api/axis-labels/text-and-multi-line-labels)
      - [Image Labels](/2d-charts/axis-api/axis-labels/image-labels)
      - [Label Style, Alignment and Positioning](/2d-charts/axis-api/axis-labels/label-style-alignment-and-positioning)
      - [Native Text Axis Labels](/2d-charts/axis-api/axis-labels/performance-considerations-native-text-axis-abels)
      - [Polar Axis Labels](/2d-charts/axis-api/axis-labels/polar-axes-labels)
      - [Axis Labels FAQs](/2d-charts/axis-api/axis-labels/axis-labels-faq)
    - **Axis Tick, Label Interval**
      - [Gridline and Label Spacing (Interval)](/2d-charts/axis-api/axis-tick-label-interval/gridline-and-label-spacing-interval)
      - [The TickProvider API](/2d-charts/axis-api/axis-tick-label-interval/tick-provider-api)
      - [Fixed Label Intervals - Static Axis Feature](/2d-charts/axis-api/axis-tick-label-interval/fixed-label-intervals-static-axis-feature)
    - **Multi Axis and Layout**
      - [Secondary and Multiple Axes](/2d-charts/axis-api/multi-axis-and-layout/secondary-and-multiple-axis-overview)
      - [Vertical Charts (Rotate, Transpose Axis)](/2d-charts/axis-api/multi-axis-and-layout/vertical-charts-rotate-transpose-axis)
      - [Inner Axis Layout](/2d-charts/axis-api/multi-axis-and-layout/inner-axis-layout)
      - [Central Axis Layout](/2d-charts/axis-api/multi-axis-and-layout/central-axis-layout)
      - [Vertically Stacked Axis Layout](/2d-charts/axis-api/multi-axis-and-layout/vertically-stacked-axis-layout)
      - [Horizontally Stacked Axis Layout](/2d-charts/axis-api/multi-axis-and-layout/horizontally-stacked-axis-layout)
      - [Advanced Options - Custom Layout Managers](/2d-charts/axis-api/multi-axis-and-layout/advanced-options-custom-layout-managers)
      - [Polar Chart Layout](/2d-charts/axis-api/multi-axis-and-layout/polar-chart-layout)
      - [Axis Offset and OverrideOffset](/2d-charts/axis-api/multi-axis-and-layout/axis-offset-and-overrideOffset)
      - [Axis Sizing](/2d-charts/axis-api/multi-axis-and-layout/axis-sizing)
    - **Axis Styling**
      - [Title, Labels, Gridlines and Axis Band Style](/2d-charts/axis-api/axis-styling/title-labels-gridlines-axis-band-style)
      - [Axis Borders and Background](/2d-charts/axis-api/axis-styling/axis-borders-and-background)
      - [Visibility of Axis Elements](/2d-charts/axis-api/axis-styling/visibility-of-axis-elements)
      - [Polar Axis Styling](/2d-charts/axis-api/axis-styling/polar-axis-styling)
    - **Misc**
      - [Axis APIs - Convert Pixel to Data Coordinates](/2d-charts/axis-api/misc/pixel-and-data-coordinates)
      - [Axis APIs – Convert Polar Pixel to Data Coordinates](/2d-charts/axis-api/misc/polar-pixel-and-data-coordinates)
  - **Animations API**
    - [Animations API Overview](/2d-charts/animations-api/animations-api-overview)
    - [Series Startup Animations](/2d-charts/animations-api/series-startup-animations)
    - [Style Transition Animations](/2d-charts/animations-api/style-transition-animations)
    - [Dataset Animations](/2d-charts/animations-api/dataset-animations)
    - [Generic Animations](/2d-charts/animations-api/generic-animations)
  - **Annotations API**
    - [Annotations API Overview](/2d-charts/annotations-api/annotations-api-overview)
    - [BoxAnnotation](/2d-charts/annotations-api/box-annotation)
    - [LineAnnotation](/2d-charts/annotations-api/line-annotation)
    - [TextAnnotation](/2d-charts/annotations-api/text-annotation)
    - [CustomAnnotation](/2d-charts/annotations-api/custom-annotation)
    - [VerticalLineAnnotation](/2d-charts/annotations-api/vertical-line-annotation)
    - [HorizontalLineAnnotation](/2d-charts/annotations-api/horizontal-line-annotation)
    - [LineArrowAnnotation](/2d-charts/annotations-api/line-arrow-annotation)
    - **AxisMarkerAnnotation**
      - [AxisMarkerAnnotation](/2d-charts/annotations-api/axis-marker-annotation/axis-marker-annotation-overview)
      - [Image AxisMarkerAnnotation](/2d-charts/annotations-api/axis-marker-annotation/image-axis-marker-annotation)
      - [SVG Axis Marker Annotation](/2d-charts/annotations-api/axis-marker-annotation/svg-axis-marker-annotation)
    - [NativeTextAnnotation](/2d-charts/annotations-api/native-text-annotation)
    - [HTML Annotations](/2d-charts/annotations-api/html-annotation)
    - [ArcAnnotation](/2d-charts/annotations-api/arc-annotation)
    - [PolarArcAnnotation](/2d-charts/annotations-api/polar-arc-annotation)
    - [PolarPointerAnnotation](/2d-charts/annotations-api/polar-pointer-annotation)
    - [Editable Annotations](/2d-charts/annotations-api/editable-annotations)
    - [Styling Annotation Selection](/2d-charts/annotations-api/styling-annotation-selection)
    - [Annotation Hover](/2d-charts/annotations-api/annotation-hover)
  - **ChartModifier API**
    - [Legend Modifier](/2d-charts/chart-modifier-api/miscellaneous-modifiers/legend-modifier)
    - [What is the ChartModifier API](/2d-charts/chart-modifier-api/chart-modifier-api-overview)
    - [Common ChartModifiers Features](/2d-charts/chart-modifier-api/common-features)
    - **Zooming and Panning**
      - [ZoomPanModifier](/2d-charts/chart-modifier-api/zooming-and-panning/zoom-pan-modifier)
      - [RubberBandXyZoomModifier](/2d-charts/chart-modifier-api/zooming-and-panning/rubber-band-xy-zoom-modifier)
      - [PinchZoomModifier](/2d-charts/chart-modifier-api/zooming-and-panning/pinch-zoom-modifier)
      - [ZoomExtentsModifier](/2d-charts/chart-modifier-api/zooming-and-panning/zoom-extents-modifier)
      - [YAxisDragModifier](/2d-charts/chart-modifier-api/zooming-and-panning/y-axis-drag-modifier)
      - [XAxisDragModifier](/2d-charts/chart-modifier-api/zooming-and-panning/x-axis-drag-modifier)
      - [MouseWheelZoomModifier](/2d-charts/chart-modifier-api/zooming-and-panning/mouse-wheel-zoom-modifier)
      - [Easy Overview charts with SciChartOverview](/2d-charts/chart-modifier-api/zooming-and-panning/overview)
    - **CursorModifier**
      - [The CursorModifier Type](/2d-charts/chart-modifier-api/cursor-modifier/cursor-modifier-overview)
      - [Formatting CursorModifier Tooltip Items](/2d-charts/chart-modifier-api/cursor-modifier/formatting-cursor-modifier-tooltip-items)
      - [Active Legends - CursorModifier output into a legend](/2d-charts/chart-modifier-api/cursor-modifier/active-legends-cursor-modifier)
      - [Customizing the CursorModifier Tooltip Container Appearance](/2d-charts/chart-modifier-api/cursor-modifier/customizing-cursor-modifier-tooltip-container-appearance)
      - [Interpolated Tooltip Values](/2d-charts/chart-modifier-api/cursor-modifier/interpolated-tooltip-values)
    - [Rollover Modifier](/2d-charts/chart-modifier-api/rollover-modifier)
    - **VerticalSliceModifier**
      - [The VerticalSliceModifier Type](/2d-charts/chart-modifier-api/vertical-slice-modifier/vertical-slice-modifier-overview)
      - [Formatting VerticalSlice Tooltip Items](/2d-charts/chart-modifier-api/vertical-slice-modifier/formatting-vertical-slice-tooltip-items)
      - [Active Legends - VerticalSliceModifier output to a Legend](/2d-charts/chart-modifier-api/vertical-slice-modifier/active-legends-vertical-slice-modifier)
      - [Customizing VerticalSliceModifier Tooltip Containers](/2d-charts/chart-modifier-api/vertical-slice-modifier/customizing-vertical-slice-tooltip-container)
    - **Polar Chart Modifiers**
      - [PolarArcZoomModifier](/2d-charts/chart-modifier-api/polar-modifiers/polar-arc-zoom-modifier)
      - [PolarCursorModifier](/2d-charts/chart-modifier-api/polar-modifiers/polar-cursor-modifier)
      - [PolarDataPointSelectionModifier](/2d-charts/chart-modifier-api/polar-modifiers/polar-data-point-selection-modifier)
      - [PolarSeriesSelectionModifier](/2d-charts/chart-modifier-api/polar-modifiers/polar-series-selection-modifier)
      - [PolarLegendModifier](/2d-charts/chart-modifier-api/polar-modifiers/polar-legend-modifier)
      - [PolarMouseWheelZoomModifier](/2d-charts/chart-modifier-api/polar-modifiers/polar-mouse-wheel-zoom-modifier)
      - [PolarPanModifier](/2d-charts/chart-modifier-api/polar-modifiers/polar-pan-modifier)
      - [PolarZoomExtentsModifier](/2d-charts/chart-modifier-api/polar-modifiers/polar-zoom-extents-modifier)
    - **Selection & Hover**
      - [Series Selection and Hover](/2d-charts/chart-modifier-api/selection/series-selection)
      - [DataPoint Selection](/2d-charts/chart-modifier-api/selection/data-point-selection)
      - [Annotation Hover](/2d-charts/chart-modifier-api/selection/annotation-hover)
    - **Custom Modifiers**
      - [Custom Chart Modifier API](/2d-charts/chart-modifier-api/custom-modifiers/custom-modifiers-overview)
      - [Detecting Clicks On Chart Parts with a Custom Modifier](/2d-charts/chart-modifier-api/custom-modifiers/detecting-clicks-on-chart-parts)
  - **Subcharts API**
    - [What is the SubCharts API?](/2d-charts/subcharts-api/subcharts-api-overview)
    - [SubCharts Positioning](/2d-charts/subcharts-api/sub-charts-positioning)
    - [SubChart Sub Surface Transparency](/2d-charts/subcharts-api/sub-chart-sub-surface-transparency)
    - [SubCharts Html Container](/2d-charts/subcharts-api/sub-charts-html-container)
    - [Combining Polar and Cartesian Charts](/2d-charts/subcharts-api/polar-and-cartesian-sub-charts)
    - [Worked Example: Re-usable Chart Groups with SubCharts](/2d-charts/subcharts-api/example-reusable-chart-groups-with-sub-charts)
    - [Worked Example: Dynamic Multi-panel charts with SubCharts](/2d-charts/subcharts-api/exampe-dynamic-multi-panel-charts-with-sub-charts)
    - [Worked Example: Resizable Multi Pane Charts with SubCharts](/2d-charts/subcharts-api/example-resizable-multi-pane-charts-with-sub-charts)
    - [Worked Example: Using SubCharts to create a Large Dashboard](/2d-charts/subcharts-api/example-using-sub-charts-to-create-large-dashboard)
  - **Styling and Theming**
    - [Chart Styling - ThemeManager API](/2d-charts/styling-and-theming/theme-manager-api)
    - [Chart Styling - Creating a Custom Theme](/2d-charts/styling-and-theming/creating-custom-theme)
    - [Chart Styling - Style Chart Parts in Code](/2d-charts/styling-and-theming/style-chart-parts-in-code)
    - [Chart Styling - Margin and Padding](/2d-charts/styling-and-theming/margin-and-padding)
    - [Series Styling - Dash Line Patterns](/2d-charts/styling-and-theming/dash-line-patterns)
    - [Chart Styling - Image, Transparent or Blurred Backgrounds](/2d-charts/styling-and-theming/image-transparent-blurred-backgrounds)
    - [Chart Styling - Theming of Wait Loader](/2d-charts/styling-and-theming/theming-of-wait-loader)
    - [CSS Classes, Ids](/2d-charts/styling-and-theming/css-classes-ids)
    - [Chart Styling - Auto Coloring](/2d-charts/styling-and-theming/auto-coloring)
    - [Chart Styling - Chart Titles](/2d-charts/styling-and-theming/chart-titles)
    - [Polar Chart Styling](/2d-charts/styling-and-theming/polar-chart-styling)
  - **Chart Synchronization APIs**
    - [Synchronizing Multiple Charts](/2d-charts/chart-synchronization-api/synchronizing-multiple-charts)
    - [Synchronizing Vertical Charts](/2d-charts/chart-synchronization-api/synchronizing-vertical-charts)
  - **Accessibility**
    - [Creating Accessible Charts](/2d-charts/accessibility/creating-accessible-charts)
    - [Voice Over](/2d-charts/accessibility/voice-over)
    - [Color and Contrast](/2d-charts/accessibility/color-and-contrast)
    - [Keyboard Accessibility](/2d-charts/accessibility/keyboard-accessibility)
  - **Miscellaneous APIs**
    - [Retina Support and Browser Zoom](/2d-charts/miscellaneous-apis/retina-support-and-browser-zoom)
    - [Batching Updates or Temporary Suspending Drawing](/2d-charts/miscellaneous-apis/batching-updates-or-temporary-suspending-drawing)
    - [Native Text Api](/2d-charts/miscellaneous-apis/native-text-api)
    - [Glow and DropShadow Shader Effects](/2d-charts/miscellaneous-apis/glow-and-dro-shadow-shader-effects)
    - [Ordered Rendering](/2d-charts/miscellaneous-apis/ordered-rendering)
    - [Render Events](/2d-charts/miscellaneous-apis/render-events)
    - [Manual Render Control](/2d-charts/miscellaneous-apis/manual-render)
  - **Performance Tips**
    - [Performance Tips & Tricks](/2d-charts/performance-tips/performance-tips-and-tricks)
    - [Memory Best Practices](/2d-charts/performance-tips/memory-best-practices)
    - [Memory Leak Debugging](/2d-charts/performance-tips/memory-leak-debugging)
- **3D Charts API**
  - [Creating a 3D Chart](/3d-charts/builder-api/default-3d-chart)
  - **SciChart3D Basics**
    - [Creating your first SciChart3DSurface](/3d-charts/scichart-3d-basics/scichart-3d-basics-overview)
    - [SciChart3DSurface.create() vs. createSingle()](/3d-charts/scichart-3d-basics/scichart-3d-surface-create-and-create-single)
    - [Coordinates in 3D Space](/3d-charts/scichart-3d-basics/coordinates-in-3d-space)
    - [The SciChartSurface Camera](/3d-charts/scichart-3d-basics/scichart-surface-camera)
  - **Axis3D APIs**
    - [Axis3D APIs Overview](/3d-charts/axis-3d-api/axis-3d-api-overview)
    - [Numeric and Date Axis in SciChart3D](/3d-charts/axis-3d-api/numeric-and-date-axis-in-scichart-3d)
    - [Axis3D Text (Label) Formatting](/3d-charts/axis-3d-api/axis-3d-text-label-formatting)
    - [3D Label Options](/3d-charts/axis-3d-api/chart-styling-3d)
    - [Axis3D Gridline and Label Spacing (Interval)](/3d-charts/axis-3d-api/axis-3d-gridline-and-label-spacing-interval)
  - **3D Chart Types**
    - [The Scatter 3D Chart Type](/3d-charts/chart-types/scatter-3d-chart)
    - [The Bubble 3D Chart Type](/3d-charts/chart-types/bubble-3d-chart)
    - [The SurfaceMesh 3D Chart Type](/3d-charts/chart-types/surface-mesh-3d)
    - [The Lines 3D Chart Type](/3d-charts/chart-types/lines-3d-chart)
    - [The Column 3D Chart Type](/3d-charts/chart-types/column-3d-chart)
  - **ChartModifier 3D API**
    - **Zooming and Panning**
      - [Orbit Modifier 3D](/3d-charts/chart-modifier-3d-api/zooming-and-panning/orbit-modifier-3d)
      - [Pinch Zoom Modifier 3D](/3d-charts/chart-modifier-3d-api/zooming-and-panning/pinch-zoom-modifier-3d)
      - [Mouse Wheel Zoom Modifier 3D](/3d-charts/chart-modifier-3d-api/zooming-and-panning/mouse-wheel-zoom-modifier-3d)
      - [Reset Camera Modifier 3D](/3d-charts/chart-modifier-3d-api/zooming-and-panning/reset-camera-modifier-3d)
    - [Tooltip Modifier 3D](/3d-charts/chart-modifier-3d-api/tooltip-modifier-3d)
<!-- generate-docs-toc.js end -->