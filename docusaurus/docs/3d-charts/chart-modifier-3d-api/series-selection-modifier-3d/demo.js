import { MouseWheelZoomModifier3D, NumberRange, NumericAxis3D, OrbitModifier3D, PointLineRenderableSeries3D, ResetCamera3DModifier, SciChart3DSurface, SciChartJsNavyTheme, SeriesSelectionModifier3D, Vector3, XyzDataSeries3D } from "scichart";
const seriesCount = 8;
const pointsPerSeries = 256;
const defaultSelectedIndex = 3;
const createWaterfallRow = (rowIndex) => {
    const xValues = [];
    const yValues = [];
    const zValues = [];
...
async function drawExample(divElementId) {
    ...
    let selectedIndex = defaultSelectedIndex;
    let hoveredIndex;
    const updateStatus = (hitInfo) => {
    ...
    updateSeriesStyles();
    updateStatus();
    sciChart3DSurface.chartModifiers.add(new MouseWheelZoomModifier3D(), new OrbitModifier3D(), new ResetCamera3DModifier(), new SeriesSelectionModifier3D({
        enableHover: true,
