...
const lightMode = document.getElementById("light-mode") as HTMLSelectElement;
const directionInputs = [input("direction-x"), input("direction-y"), input("direction-z")];

const setOutput = (control: HTMLInputElement) => {
...
async function createLightingDemo() {
    ...
    const size = 30;
    const surfaceValues = Array.from({ length: size }, (_, z) =>
        Array.from({ length: size }, (_, x) => 4 + 7 * Math.sin(x * 0.25) * Math.cos(z * 0.22) + 5 * Math.exp(-((x - 15) ** 2 + (z - 15) ** 2) / 70))
    );
    ...
    const applyLighting = () => {
        ...
        const mode = lightMode.value as EMainLightMode;
