...
async function drawExample(divElementId) {
    // #region_A_start
    const { NumberRange, NumericAxis, SciChartSurface, EVerticalTextPosition, AnnotationHoverModifier, ECursorStyle } = SciChart; // or import from "scichart"
    const { EFibonacciLabelColorMode, EFibonacciLabelPlacement, EMultiPointLabelAnchorMode, FibonacciRetracementAnnotation, SciTraderLightTheme } = SciChartFinancialTools; // if using npm, import from "scichart-financial-tools";
    ...
    sciChartSurface.annotations.add(new FibonacciRetracementAnnotation({
        ...
        regionColors: [
            "#26F",
            "#F62", // if there are more regions than colors, we'll interpolate the rest based on these for you!
        ],
