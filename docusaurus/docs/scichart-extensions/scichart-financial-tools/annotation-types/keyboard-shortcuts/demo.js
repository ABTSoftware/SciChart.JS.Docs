...
async function drawExample(divElementId) {
    // #region_A_start
    const { AnnotationHoverModifier, ECursorStyle, EHorizontalAnchorPoint, EVerticalAnchorPoint, NativeTextAnnotation, NumberRange, NumericAxis, SciChartSurface } = SciChart;
    ...
    sciChartSurface.annotations.add(new NativeTextAnnotation({
        ...
        { x: 22, y: 129 },
        { x: 34, y: 121 }
    ], "#3B82F6", true), createEditableLine([
        { x: 10, y: 112 },
        { x: 23, y: 118 },
        { x: 36, y: 110 }
    ], "#F97316"), createEditableLine([
        { x: 10, y: 100 },
        { x: 24, y: 106 },
        { x: 38, y: 97 }
    ], "#10B981"));
    ...
    const isTypingTarget = target => target instanceof HTMLElement && (target.isContentEditable || /^(INPUT|TEXTAREA|SELECT)$/.test(target.tagName));
