...
async function drawExample(divElementId) {
    ...
    const { EFibonacciLabelColorMode, EFibonacciLabelPlacement, FibonacciRetracementAnnotation, SciTraderLightTheme } =
    ...
    document.querySelector<HTMLInputElement>("#extend-start")!.addEventListener("change", event => {
        retracement.extendStart = (event.target as HTMLInputElement).checked;
        document.querySelector("#extend-start-label")!.textContent = `extendStart:\u00A0${retracement.extendStart}`;
    });
    document.querySelector<HTMLInputElement>("#extend-end")!.addEventListener("change", event => {
        retracement.extendEnd = (event.target as HTMLInputElement).checked;
