...
async function drawExample(divElementId) {
    // #region_A_start
    const { NumberRange, NumericAxis, SciChartSurface, toEngineering } = SciChart; // or import from "scichart"
    const { ChannelAnnotation, EAnnotationVisibilityMode, EMultiPointLabelAnchorMode, ExtendedLineAnnotation, SciTraderLightTheme, StopLossTakeProfitAnnotation } = SciChartFinancialTools; // if using npm, import from "scichart-financial-tools";
    ...
    const trendLabelStyle = ({ annotation, label, defaultStyle }) => {
        ...
        if (annotation instanceof ChannelAnnotation && label.anchorMode === EMultiPointLabelAnchorMode.Point) {
            ...
            if (label.id === "channel-upper")
                return { ...defaultStyle, fontSize: offsetAbove ? 13 : 0 };
            if (label.id === "channel-lower")
                return { ...defaultStyle, fontSize: offsetAbove ? 0 : 13 };
        }
        return defaultStyle;
    };
