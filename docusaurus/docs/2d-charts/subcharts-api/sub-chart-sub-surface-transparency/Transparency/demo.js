...
async function simpleSubChart(divElementId) {
    ...
    const { SciChartSurface, SciChartSubSurface, NumericAxis, SciChartJsNavyTheme, Rect, ECoordinateMode, FastLineRenderableSeries, XyDataSeries, MouseWheelZoomModifier, ZoomPanModifier } = SciChart;
    ...
    // Add some random data to the sub chart so it can be shown through the subchart
    subChart2.renderableSeries.add(new FastLineRenderableSeries(wasmContext, {
        strokeThickness: 5,
        stroke: "Orange",
        dataSeries: new XyDataSeries(wasmContext, { xValues, yValues: yValues1 })
    }));
