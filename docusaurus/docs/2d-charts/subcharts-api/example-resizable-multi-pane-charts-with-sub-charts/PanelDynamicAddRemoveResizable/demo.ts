...
function updateSplitterPositions() {
    const splitters = document.querySelectorAll<HTMLDivElement>(".grid-splitter");
    let currentY = 0;
...
function setupSplitterEvents(splitter, parentSciChartSurface, axisSynchronizer) {
    ...
    splitter.addEventListener("pointermove", e => {
        ...
        if (mouseY >= minY && mouseY <= maxY) {
            ...
            const splitters = Array.from(document.querySelectorAll<HTMLDivElement>(".grid-splitter"));
