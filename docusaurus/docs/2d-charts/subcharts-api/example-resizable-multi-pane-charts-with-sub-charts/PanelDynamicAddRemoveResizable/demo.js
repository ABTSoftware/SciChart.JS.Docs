import { SciChartSurface, NumericAxis, SciChartJsNavyTheme, Rect, ZoomPanModifier, MouseWheelZoomModifier, ZoomExtentsModifier, EventHandler, XyDataSeries, FastLineRenderableSeries, NumberRange, SciChartSubSurface } from "scichart";
let colorIndex = 0;
...
// #region_AxisSynchroniser_end
// #region_gridSplitterHelperFunctions_start
...
function setupSplitterEvents(splitter, parentSciChartSurface, axisSynchronizer) {
    ...
    splitter.addEventListener("pointermove", e => {
        ...
        if (mouseY >= minY && mouseY <= maxY) {
            ...
            const prevSplitterPosition = currentSplitterIndex > 0 ? parseFloat(splitters[currentSplitterIndex - 1].style.top) / 100 : 0;
            const nextSplitterPosition = currentSplitterIndex < splitters.length - 1
            ...
            const upperPanelStart = upperPanelIndex > 0 ? panelSizes.slice(0, upperPanelIndex).reduce((a, b) => a + b, 0) : 0;
...
// #region_gridSplitterHelperFunctions_end
// #region_addNewChart_start
...
function addNewChart(parentSciChartSurface, wasmContext, axisSynchronizer) {
    ...
    newChart.yAxes.add(new NumericAxis(wasmContext, {
        axisTitle: "YAxis",
        axisTitleStyle: { fontSize: 12 }
    }));
    // Add modifiers
...
// #region_addNewChart_end
// #region_removeChart_start
...
function removeSpecificChart(index, parentSciChartSurface, axisSynchronizer) {
    const chartCount = parentSciChartSurface.subCharts?.length ?? 0;
    if (chartCount <= 1)
        return; // Keep at least one chart
...
// #region_removeChart_end
// #region_createDynamicPanelChart_start
...
// #region_createDynamicPanelChart_end
