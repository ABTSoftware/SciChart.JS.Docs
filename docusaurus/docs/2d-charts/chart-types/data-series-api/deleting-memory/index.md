...
:::note
See related articles: [Memory Usage Best Practices](/2d-charts/performance-tips/memory-best-practices/) which has some further info on optimising memory usage with SciChart.js, and [Memory Leak Debugging](/2d-charts/performance-tips/memory-leak-debugging/) which introduces our new tools to identify and fix memory leaks.
:::

Examples of how to use Delete()
-------------------------------

Once you are finished with the DataSeries, don't forget to call [IDeletable.delete():blue_book:](https://www.scichart.com/documentation/js/v5/typedoc/interfaces/ideletable.html#delete). This frees WebAssembly native memory and releases it back to the host.
...
```
:::warning
Failing to call [IDeletable.delete():blue_book:](https://www.scichart.com/documentation/js/v5/typedoc/interfaces/ideletable.html#delete) on a DataSeries or it's parent SciChartSurface when it is no longer needed can result in a memory leak.
...
- [Memory Usage Best Practices](/2d-charts/performance-tips/memory-best-practices/)
