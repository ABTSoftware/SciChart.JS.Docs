...
async function candlestickAndVolumeChart(divElementId) {
    ...
    const { SciChartSurface, CategoryAxis, NumericAxis, FastCandlestickRenderableSeries, FastColumnRenderableSeries, XyDataSeries, OhlcDataSeries, NumberRange, SciChartJsNavyTheme } = SciChart;
    ...
    const { dateValues, openValues, highValues, lowValues, closeValues, volumeValues } = await getCandles("BTCUSDT", "4h", 70);
...
async function builderExample(divElementId) {
    ...
    const { build2DChart, registerType, ESeriesType, EAxisType, EThemeProviderType, NumberRange, EBaseType, EPaletteProviderType } = SciChart;
