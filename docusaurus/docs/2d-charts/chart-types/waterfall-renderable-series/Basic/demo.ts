...
class WaterfallPaletteProvider extends DefaultPaletteProvider {
    ...
    overrideFillArgb(_x: number, _y: number, _index: number, _opacity: number, metadata: any): number {
        const profit = metadata?.profit;
        if (profit === 0) return parseColorToUIntArgb("#2196F3", 200); // Total bar
        return profit > 0
...
class WaterfallDataLabelProvider extends RectangleSeriesDataLabelProvider {
    getText(metadataSelector: any): string {
...
async function waterfallChart(divElementId: string) {
    // #region_A_start
