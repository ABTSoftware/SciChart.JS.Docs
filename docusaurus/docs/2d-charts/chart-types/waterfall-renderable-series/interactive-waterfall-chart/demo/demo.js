...
function generateSpectralData(sliceIndex, spectraSize) {
    const xValues = [];
    const yValues = [];
...
async function interactiveWaterfallChart(divElementId) {
    // #region_A_start
    ...
    const { SciChartSurface, NumericAxis, FastLineRenderableSeries, XyDataSeries, EAxisAlignment, EXyDirection, NumberRange, MouseWheelZoomModifier, ZoomExtentsModifier, ZoomPanModifier, SeriesSelectionModifier, SciChartJsNavyTheme, } = SciChart;
    ...
    }), new ZoomPanModifier({ enableZoom: true, xyDirection: EXyDirection.XDirection }), new MouseWheelZoomModifier({ xyDirection: EXyDirection.XDirection }), new ZoomExtentsModifier({ xyDirection: EXyDirection.XDirection }));
    // #region_A_end
}
