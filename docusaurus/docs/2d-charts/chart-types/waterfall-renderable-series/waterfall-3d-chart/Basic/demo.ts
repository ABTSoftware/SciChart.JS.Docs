...
function generateSpectralSlice(sliceIndex: number, spectraSize: number) {
...
function valueToColor(value: number, low: number, high: number): number {
    const gradientStops = [
    ...
    if (i <= 0) return parseColorToUIntArgb(gradientStops[0].color);
...
async function waterfallChart3D(divElementId: string) {
    // #region_A_start
