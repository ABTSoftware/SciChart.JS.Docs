import { ContoursDataLabelProvider, DpiHelper, formatNumber, HeatmapColorMap, MouseWheelZoomModifier, NumericAxis, parseColorToUIntArgb, Point, Rect, SciChartJsNavyTheme, SciChartSurface, UniformContoursRenderableSeries, UniformHeatmapDataSeries, zeroArray2D, ZoomExtentsModifier, ZoomPanModifier } from "scichart";
/** Where between two samples the contour crosses */
const crossing = (from, to, level) => (level - from) / (to - from);
/**
 ...
 */
const traceContourSegments = (zValues, levels) => {
    const segments = [];
    ...
    for (let j = 0; j < height - 1; j++) {
        for (let i = 0; i < width - 1; i++) {
            ...
            for (const level of levels) {
                ...
                const add = (from, to) => segments.push({ level, i0: from.i, j0: from.j, i1: to.i, j1: to.j });
                // Which corners are above the level decides which edges the line enters and leaves by
                const bits = (v00 > level ? 1 : 0) | (v10 > level ? 2 : 0) | (v11 > level ? 4 : 0) | (v01 > level ? 8 : 0);
                switch (bits) {
                    case 1:
                    case 14:
                        add(left(), bottom());
...
export class AlongLineContoursDataLabelProvider extends ContoursDataLabelProvider {
    ...
    generateDataLabels(renderContext, renderPassData) {
        this.dataLabels = [];
        if (!this.isEnabled || !this.style?.fontFamily || !this.style?.fontSize)
            return;
        ...
        const levels = params.zLevels?.length
        ...
        for (const segment of segments) {
            ...
            const tooClose = placed.some(other => other.level === segment.level &&
            ...
            if (rect.x < 0 || rect.y < 0 || rect.right > viewRect.width || rect.bottom > viewRect.height)
                continue;
            if (placed.some(other => rectsOverlap(other.rect, rect)))
                continue;
...
const rectsOverlap = (first, second) => first.left < second.right && second.left < first.right && first.top < second.bottom && second.top < first.bottom;
// #region_provider_end
/** A two-well potential: near-circular level sets around each well, with a saddle between them */
function generateExampleData(width, height) {
...
const createChart = async (divElementId, dataLabelProvider) => {
...
const drawExample = async () => {
    ...
    await createChart("scichart-root", new ContoursDataLabelProvider({
        style: { fontSize: 12 },
        color: "#FFFFFF",
        labelRowCount: 3,
        precision: 0
    ...
    await createChart("scichart-root-2", new AlongLineContoursDataLabelProvider({
        style: { fontSize: 12 },
        color: "#FFFFFF",
        labelSpacing: 130,
        rotateToLine: true,
