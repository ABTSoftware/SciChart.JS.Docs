...
async function drawMountainChartWithPalette(divElementId) {
    ...
    const { SciChartSurface, NumericAxis, FastMountainRenderableSeries, GradientParams, XyDataSeries, Point, SciChartJsNavyTheme } = SciChart;
...
async function builderExample(divElementId) {
    ...
    chartBuilder.registerType(EBaseType.PaletteProvider, "MountainPaletteProvider", options => new MountainPaletteProvider(options.threshold));
    // Create some data
    let yLast = 100.0;
    const xValues = [];
    const yValues = [];
