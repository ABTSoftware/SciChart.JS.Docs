...
function generateExampleData(width, height, cpMax, index, maxIndex) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            const v = (1 + Math.sin(x * 0.02 + angle)) * 50 +
...
async function dynamicColorMaps(divElementIdChart, divElementIdLegend) {
    ...
    const { SciChartSurface, NumericAxis, HeatmapColorMap, UniformHeatmapDataSeries, UniformHeatmapRenderableSeries, SciChartJsNavyTheme, NumberRange, HeatmapLegend, AxisMarkerAnnotation, TextAnnotation, BoxAnnotation, ECoordinateMode, EHorizontalAnchorPoint, EVerticalAnchorPoint } = SciChart;
    ...
    const axisAnnotation = new AxisMarkerAnnotation({
        ...
        onDrag: args => {
            ...
            if (axisAnnotation.y1 > 200)
                axisAnnotation.y1 = 200;
            if (axisAnnotation.y1 < 0)
                axisAnnotation.y1 = 0;
