import { BottomAlignedOuterHorizontallyStackedAxisLayoutStrategy, BottomAlignedOuterAxisLayoutStrategy, getHorizontalAxisRequiredSize, SciChartSurface, NumericAxis, SciChartJsNavyTheme, ZoomPanModifier, PinchZoomModifier, ZoomExtentsModifier, MouseWheelZoomModifier, FastLineRenderableSeries, XyDataSeries, TextAnnotation } from "scichart";
// #region_A_start
...
class CustomAxisLayoutStrategy extends BottomAlignedOuterHorizontallyStackedAxisLayoutStrategy {
    ...
    measureAxes(sciChartSurface, chartLayoutState, axes) {
        ...
        const stackedAreaSize = this.defaultBottomOuterAxisLayoutStrategy.measureAxes(sciChartSurface, chartLayoutState, [firstAxis]);
...
async function customLayoutManager(divElementId) {
    ...
    sciChartSurface.chartModifiers.add(new ZoomPanModifier(), new PinchZoomModifier(), new ZoomExtentsModifier(), new MouseWheelZoomModifier({ applyToSeriesViewRect: false }));
}
